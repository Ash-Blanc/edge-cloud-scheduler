...
def spt_mean(ps, releases=None):
    """Non-preemptive SPT mean completion on one machine.
    If releases is None, all jobs available at 0."""
    n = len(ps)
    if n == 0:
        return 0.0
    if releases is None:
        ...
        for i in order:
            ...
            acc += t
        return acc / n
    # Release times: SRPT-style at the single-machine relaxation (preemptive
    # shortest remaining is the mean-C optimum). Remaining starts at p_i.
    rem = list(ps)
    rel = list(releases)
    t = 0.0
    done = 0.0
    ...
    finished = 0
    while finished < n:
        avail = [i for i in range(n) if rel[i] <= t + 1e-15 and rem[i] > 1e-15]
        if not avail:
            t = min(rel[i] for i in range(n) if rem[i] > 1e-15)
            continue
        j = min(avail, key=lambda i: rem[i])
        nxt = min((rel[i] for i in range(n) if rem[i] > 1e-15 and rel[i] > t + 1e-15),
                  default=t + rem[j])
        step = min(rem[j], nxt - t)
        if step <= 1e-15:
            step = rem[j]
        rem[j] -= step
        t += step
...
def mean_completion_lbs(c):
    ...
    # Identical-release SPT on each resource, plus the rest of the chain which
    # the same job still has to pay after that resource (cannot overlap itself).
    edge_p = [j["ppre"] + j["ppost"] for j in jobs]
    cloud_p = [j["pproc"] for j in jobs]
    link_p = [j["up"] for j in jobs]
    rel = [j["arr"] for j in jobs]
    edge_spt = spt_mean(edge_p, rel)
    # K parallel clouds: scale times by assigning each job to a machine in
    # SPT order (list scheduling lower-bounded by p/K on a single speed-K).
    cloud_speedk = spt_mean([p / max(c.K, 1) for p in cloud_p], rel)
    up_spt = spt_mean(link_p, rel)
    down_spt = up_spt
    # After the bottleneck, the job still owes the other stages. Using the
    # mean of the complementary chain is valid (linearity) and slightly weak.
    rest_edge = sum(j["pproc"] + 2 * j["up"] for j in jobs) / n
    rest_cloud = sum(j["ppre"] + j["ppost"] + 2 * j["up"] for j in jobs) / n
    rest_up = sum(j["ppre"] + j["pproc"] + j["ppost"] + j["up"] for j in jobs) / n
    lbs = {
        "chain": chain,
        "edge_spt+rest": edge_spt + rest_edge,
        "cloud_spt+rest": cloud_speedk + rest_cloud,
        "up_spt+rest": up_spt + rest_up,
        "down_spt+rest": down_spt + rest_up,
        "edge_spt": edge_spt,
        "cloud_spt": cloud_speedk,
        "up_spt": up_spt,
    }
    lbs["best"] = max(lbs[k] for k in
                      ("chain", "edge_spt+rest", "cloud_spt+rest",
                       "up_spt+rest", "down_spt+rest"))
    lbs["which"] = max(
        ("chain", "edge_spt+rest", "cloud_spt+rest", "up_spt+rest", "down_spt+rest"),
        key=lambda k: lbs[k],
    )
    return lbs
...
def official15_cases(seeds):
    ...
    for seed in seeds:
        ...
        arr2 = sorted(
            (rng2.uniform(0.0, 2.0), rng2.choice([2048, 4096]), 1)
            for _ in range(2000)
        )
        c2 = sim.Case(1, 2.0, 1.0, 0.05, 32768, 8,
                      1.0, 1.0, sim.make_table("cpu", rng2), arr2, .45, .55)
        c2.name = f"official15-link-s{seed}"
        c2.tp_base, c2.tp_ub, c2.dist_base = 1e-8, 1e-4, 80.0
        out.append(c2)
    return out
...
def main():
    ...
    print("\n  floor grid (subset) for current binary:")
    bad = 0
    worst = (0.0, 0.0, "")
    n = 0
    ...
    for c in official10_cases((1009, 1010, 1011, 1012)):
        ...
        print(f"    {c.name}: meanTDR={tdr:.1f}  LB[{lbs['which']}]={lbs['best']:.1f}  "
              ...
              f"edge={lbs['edge_spt']:.1f} cloud={lbs['cloud_spt']:.1f} up={lbs['up_spt']:.1f}")
        try:
            ma, _, _, _ = report_run("akd", AKD, c)
            print(f"    vs AKD tdr {ma[1]:.1f}->{tdr:.1f} ({tdr/ma[1]:.4f}x)  "
                  f"tp {ma[0]:.6g}->{m[0]:.6g} ({m[0]/ma[0]:.6f}x)")
        except Exception as e:
            print(f"    akd FAIL {e}")
    ...
    for c in official15_cases((1509, 1510, 1511)):
        lbs = mean_completion_lbs(c)
        m, frames, sm, pts = report_run("current", BASE, c)
        tdr = m[1]
        print(f"    {c.name}: meanTDR={tdr:.1f}  LB[{lbs['which']}]={lbs['best']:.1f}  "
