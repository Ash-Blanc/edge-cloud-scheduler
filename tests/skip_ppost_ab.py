...
VARIANTS = [
    ("cur", []),
    ("sk90", ["AKD56_SKIP_PPOST_90=1"]),
    ("lptP", ["AKD56_PPROC_LPT_90=1"]),
    ("sklpt", ["AKD56_SKIP_PPOST_90=1", "AKD56_PPROC_LPT_90=1"]),
    ("dfsk", ["AKD56_DPROC_FIRST=1", "AKD56_SKIP_PPOST_90=1"]),
]

def build(name, defs):
    out = os.path.join(BIN, f"sk_{name}.exe")
    r = subprocess.run(
...
def main():
    ...
    for name, defs in VARIANTS:
        ...
        d5, d6 = p5 - b5, p6 - b6
        print(f"{name:<8} d5={d5:+7.2f} d6={d6:+7.2f} tp5={m5[0]:.4f} tp6={m6[0]:.4f} "
              f"tdr6={m6[1]:.0f} tpot6={m6[2]:.2f}")
