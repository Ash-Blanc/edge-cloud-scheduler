#include <bits/stdc++.h>
     ...
     nPrefPend++;
     sumArrPend += now;
     double w = tPpre.get(r.lin) + tPproc.get(r.lin) + tPpost.get(r.lin) + 2.0 * xfer(r.lin);
     ...
     searchCap = hi;
     if (WC > 1e-9 && gapPredict(1) <= SLO2) {
     if (DBASE <= 0 || pureLat || (WC >= 0.85 && DBASE > 0)) {
     searchCap = 1;
     for (int m = 1; m <= hi; ++m) {
     ...
     const int readyDecodeCount = (int)BK[B_FRESH].size() + (int)BK[B_ACT].size();
     const bool priorityDecodeReady =
     akd56Cap && throughputPriority && (readyDecodeCount >= max(1, mDesign));
     ...
     (!singleFlightDecode || decodeRoundsInFlight == 0)) {
     if (nPrefPend == 0 && ready > 0 && ready == nLive && ready < mDesign) {
     fire = true;
     } else if (historical22Mode && ready >= mDesign) {
     fire = true;
     ...
     if (fire) {
     batch.clear();
     if (nPrefPend == 0 && ready == nLive && ready < mDesign) {
     for (int rid : BK[B_ACT]) batch.push_back(rid);
     for (int rid : BK[B_FRESH]) batch.push_back(rid);
     } else if (pipeStagger) {
     int seats = mDesign;
